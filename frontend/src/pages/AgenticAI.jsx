  ...
  return (
    <div>
      <Hero
        ...
        imageSrc="https://lh3.googleusercontent.com/aida-public/AB6AXuAcvkb8eDb4gdAyYE5srRl985HxUQacvPrAPDCqGWLHlqUJ0hxslod8-hjlaeOKXikfWcj5XYZCxf0M4Z_Vqa4Dw3iMRzFZKnEYQByrU6cZDXj2dPzvdJF68bmSV5fftxPx9qvsghyja1HzFzEjKBI3Kt1fNn0fsy-v6LYJPWNVydZjBiGJIJO1OJ0H7m-17KbmKk5ZsrvQf01IjQzoexDu1sqrRHhKS6IbFCVrUBAgUG-NR1CnPC3P2n6CM4VjyMtFOCLJSkz5w"
      />
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-3 gap-6 md:gap-8 items-center">
          <Card className="p-8 opacity-60 order-2 lg:order-1 w-full">
            ...
            <ul className="space-y-4">
              <li className="flex items-center text-sm text-white/40 italic"><span className="mr-2">x</span> Fixed if-then rules</li>
              <li className="flex items-center text-sm text-white/40 italic"><span className="mr-2">x</span> Manual updates required</li>
              <li className="flex items-center text-sm text-white/40 italic"><span className="mr-2">x</span> Single system isolation</li>
            </ul>
          </Card>
              ...
              <summary className="list-none cursor-pointer p-5 flex items-center justify-between gap-4">
                <div className="flex items-center gap-4 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-lg font-heading font-bold text-white truncate">{cap.title}</h3>
                    <p className="text-xs text-white/50 truncate">{cap.tag}</p>
                  </div>
                </div>
                <span className="text-white/40 text-2xl leading-none">+</span>
              </summary>
              <div className="px-5 pb-5 pt-1 border-t border-white/5">
                <p className="text-white/50 text-sm mb-4 leading-relaxed">{cap.desc}</p>
      ...
      <Section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          <Card className="aspect-auto md:aspect-video relative overflow-hidden group p-0">
            <div className="absolute inset-0 bg-gradient-to-br from-[#0F1E35] via-background to-[#08111f]"></div>
            <div className="relative p-6 md:p-10 h-full flex flex-col justify-between min-h-[280px] md:min-h-0">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-white/40">Use Case Loop</span>
              </div>
              <div className="space-y-4">
                <h4 className="text-2xl md:text-3xl font-heading font-bold text-white">Conversation first, not image first</h4>
                <p className="text-white/50 text-sm md:text-base leading-relaxed max-w-md">This panel stays readable on small screens without the oversized message image, keeping the focus on the selected workflow.</p>
              </div>
          ...
          <div className="space-y-8">
            ...
            <p className="text-white/50 text-lg leading-relaxed">Replace traditional scripted support with agents that resolve complex disputes, process returns, and manage scheduling without a single human touchpoint.</p>
            <ul className="space-y-4">
              {['24/7 Global multi-lingual support', 'Direct integration with CRM & Logistics', 'Real-time tone & sentiment adaptation'].map((item, i) => (
                <li key={i} className="flex items-center gap-3 text-white/70 font-medium"><span className="text-primary">v</span> {item}</li>
              ))}
            </ul>
